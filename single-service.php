<?php
/**
 * Single Service — قالب واحد ثابت لجميع الخدمات.
 * أي خدمة (حالية أو جديدة من الداشبورد) تظهر بنفس الشكل: هيرو + إحصائيات + محتوى (service-landing).
 */
get_header(); ?>
...
<?php while (have_posts()) :
    ...
    // Determine the default asset image based on service type
    $slug = get_post_field('post_name', get_the_ID());
    $title_lower = mb_strtolower(get_the_title());
    $default_asset = 'hero-bg.jpg'; // Default fallback

    if (strpos($slug, 'interior') !== false || strpos($title_lower, 'داخلي') !== false) {
        $default_asset = 'service-interior.jpg';
    } elseif (strpos($slug, 'architectural') !== false || strpos($title_lower, 'معماري') !== false) {
        $default_asset = 'service-architectural.jpg';
    } elseif (strpos($slug, 'mep') !== false || strpos($title_lower, 'mep') !== false) {
        $default_asset = 'service-mep.jpg';
    } elseif (strpos($slug, 'survey') !== false || strpos($title_lower, 'مساحة') !== false) {
        $default_asset = 'service-surveying.jpg';
    }
...
?>
    ...
    <section class="relative h-[60vh] min-h-[500px] flex items-center justify-center overflow-hidden bg-slate-800">
        ...
        <div class="relative z-10 container px-4 sm:px-6 text-center text-white">
            ...
            <h1 class="text-4xl md:text-6xl lg:text-7xl font-bold mb-3 leading-tight animate-fade-in-up delay-100">
                <?php
                // عنوان مخصص لخدمة هندسة المساحة والجيوماتكس
                if (strpos($slug, 'survey') !== false || strpos($title_lower, 'مساحة') !== false) {
                    echo 'هندسة المساحة والجيوماتكس';
                } else {
                    the_title();
                }
                ?>
            ...
            <?php
            // وصف مخصص أسفل العنوان لخدمة هندسة المساحة والجيوماتكس
            if (strpos($slug, 'survey') !== false || strpos($title_lower, 'مساحة') !== false) : ?>
                <p class="text-base md:text-lg text-blue-100 max-w-2xl mx-auto mb-8 animate-fade-in-up delay-200">
                    حلول مساحية شاملة.. من الرصد الأرضي إلى الفضاء
                </p>
            <?php elseif ($hero_desc) : ?>
                <p class="text-lg md:text-xl text-blue-100 max-w-2xl mx-auto mb-8 animate-fade-in-up delay-200">
            ...
            <div class="flex flex-wrap justify-center gap-4 animate-fade-in-up delay-300">
                ...
                <a href="<?php echo esc_url(home_url('/quotation')); ?>" class="px-8 py-4 bg-white/10 backdrop-blur-sm border border-white/30 text-white rounded-xl font-bold hover:bg-white/20 transition">
                    طلب عرض سعر
                </a>
            </div>
        </div>
        ...
        <?php if (function_exists('have_rows') && have_rows('service_stats')) : ?>
            <?php while (have_rows('service_stats')) : the_row(); ?>
                <div class="text-center py-4 md:py-0">
                    <div class="text-4xl lg:text-5xl font-extrabold text-blue-600 mb-2 stat-counter" data-end="<?php echo (int)filter_var(get_sub_field('value'), FILTER_SANITIZE_NUMBER_INT); ?>" data-suffix="<?php echo filter_var(get_sub_field('value'), FILTER_SANITIZE_NUMBER_INT) ? '+' : ''; ?>">
...
<?php
    // تحميل قالب المحتوى حسب نوع الخدمة: مساحة → survey، MEP → mep، معماري → architectural، داخلي → interior-design، والباقي → landing
    $current_service_id = get_the_ID();
    set_query_var('current_service_id', $current_service_id);
    $service_slug = get_post_field('post_name', $current_service_id);
    $service_part = 'landing';
    if (strpos($service_slug, 'survey') !== false || strpos($service_slug, 'مساحة') !== false || mb_strpos($title_lower, 'مساحة') !== false) {
        $service_part = 'survey';
    } elseif (strpos($service_slug, 'mep') !== false || mb_strpos($title_lower, 'mep') !== false || mb_strpos($title_lower, 'إليكترو') !== false) {
        $service_part = 'mep';
    } elseif (strpos($service_slug, 'architectural') !== false || mb_strpos($title_lower, 'معماري') !== false || mb_strpos($title_lower, 'إنشائي') !== false) {
        $service_part = 'architectural';
    } elseif (strpos($service_slug, 'interior') !== false || mb_strpos($title_lower, 'داخلي') !== false) {
        $service_part = 'interior-design';
    }
    get_template_part('template-parts/service', $service_part);
?>
